# sirtosh-taksi-bot
Sirdaryo va Toshkent yo'nalishi uchun taksi agregatori boti`2400075972291
import logging
import asyncio
...
# SIZ BERGAN REAL BOT TOKEN SHU YERGA JOYLASHTIRILDI:
...
# Vaqtincha ma'lumotlar bazasi (Real loyihada SQLite yoki PostgreSQL bo'ladi)
DRIVERS = {} 
...
async def dr_photo(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        # Agar admin ID noto'g'ri bo'lsa xatolik bermasligi uchun
        pass
    await state.clear()
...
async def client_tariff(message: types.Message, state: FSMContext):
    tariff_clean = message.text.replace("Standart", "Standart").replace("✨ ", "")
